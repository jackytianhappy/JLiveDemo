...
@interface JRTMPSession ()<JStreamSessionDelegate>
{
...
@property (nonatomic,strong) JStreamSession *streamSession;
@property (nonatomic,assign) JRtmpSessionStatus rtmpStatus;
...
- (void)dealloc{
    ...
    _packageQueue = nil;
    _sendQueue = nil;
    _rtmpStatus = JRtmpSessionStatusNone;
    _numOfInvokes = 0;
...
- (JStreamSession *)streamSession{
    if (_streamSession == nil) {
        _streamSession = [[JStreamSession alloc] init];
        _streamSession.delegate = self;
    }
    return _streamSession;
}

- (void)setConfig:(JRTMPConfig *)config{
    _config = config;
    self.url = config.url;
}
...
- (void)setRtmpStatus:(JRtmpSessionStatus)rtmpStatus{
...
- (void)reset{
    ...
    self.rtmpStatus = JRtmpSessionStatusNone;
}
...
- (void)streamSession:(JStreamSession *)session didChangeStatus:(JStreamStatus)streamStatus{
    ...
    if (streamStatus & NSStreamEventHasSpaceAvailable){ //可以写数据
        ...
        if (_rtmpStatus == JRtmpSessionStatusConnected) {
            [self handshake0];
        }
    ...
    if ((streamStatus & NSStreamEventOpenCompleted) &&
        _rtmpStatus < JRtmpSessionStatusConnected) {
        self.rtmpStatus = JRtmpSessionStatusConnected;
    }
    
    if (streamStatus & NSStreamEventErrorOccurred) {
        self.rtmpStatus = JRtmpSessionStatusError;
    }
    
    if (streamStatus & NSStreamEventEndEncountered) {
        self.rtmpStatus = JRtmpSessionStatusNotConnected;
    }
}

- (void)handshake0{
    
    self.rtmpStatus = JRtmpSessionStatusHandshake0;
...
- (void)handshake1{
    self.rtmpStatus = JRtmpSessionStatusHandshake2;
...
- (void)sendConnectPacket{
    ...
    metadata.msg_stream_id = JStreamIDInvoke;
    metadata.msg_type_id = JMSGTypeID_INVOKE;
...
- (void)didReceivedata{
    ...
    if (self.rtmpStatus >= JRtmpSessionStatusConnected &&
        self.rtmpStatus < JRtmpSessionStatusHandshakeComplete) {
    ...
    switch (_rtmpStatus) {
        case JRtmpSessionStatusHandshake0:{
            uint8_t s0;
            [data getBytes:&s0 length:1];
            if (s0 == 0x03) {//s0
                self.rtmpStatus = JRtmpSessionStatusHandshake1;
        ...
        case JRtmpSessionStatusHandshake1:{
        ...
        case JRtmpSessionStatusHandshake2:{//s2
...
- (BOOL)handleMeesage:(uint8_t *)p :(uint8_t)msgTypeId{
    ...
    switch(msgTypeId) {
        case JMSGTypeID_BYTES_READ:
        {
            
        }
            break;
            
        case JMSGTypeID_CHUNK_SIZE:
        {
        ...
        case JMSGTypeID_AUDIO:
        {
            NSLog(@"received audio");
        }
            break;
            
        case JMSGTypeID_METADATA:
        {
            NSLog(@"received metadata");
        }
            break;
            
        case JMSGTypeID_NOTIFY:
        {
...
- (void)sendPong{
    dispatch_sync(_packageQueue, ^{
        ...
        [data appendByte24:0];
        [data appendByte24:6];
        [data appendByte:JMSGTypeID_PING];
...
- (void)handleInvoke:(uint8_t *)p{
    ...
    if ([command isEqualToString:@"_result"]) {
        ...
        if ([trackedCommand isEqualToString:@"connect"]) {
            if (self.currentActor == JDirectorActor) {
                [self sendReleaseStream];
                [self sendFCPublish];
                [self sendCreateStream];
            }
            else if (self.currentActor == JAudienceActor){
            ...
            self.rtmpStatus = JRtmpSessionStatusFCPublish;
            ...
            self.rtmpStatus = JRtmpSessionStatusReady;
        }
        ...
        if ([code isEqualToString:@"NetStream.Publish.Start"]) {
            ...
            self.rtmpStatus = JRtmpSessionStatusSessionStarted;
        }
            ...
            //sendSetBufferTime(0);//设定时间
            self.rtmpStatus = JRtmpSessionStatusSessionStartPlay;
        }
    }
}
...
- (void)sendHeaderPacket{
    ...
    metadata.msg_stream_id = JStreamIDAudio;
...
- (void)sendSetChunkSize:(int32_t)newChunkSize{
    ...
    dispatch_sync(_packageQueue, ^{
        ...
        [data appendByte24:0];
        [data appendByte24:4];
        [data appendByte:JMSGTypeID_CHUNK_SIZE];
...
- (void)sendReleaseStream{
    ...
    RTMPChunk_0 metadata = {0};
    metadata.msg_stream_id = JStreamIDInvoke;
    metadata.msg_type_id = JMSGTypeID_NOTIFY;
...
- (void)sendBufferLength{
    ...
    RTMPChunk_0 metadata = {0};
    metadata.msg_stream_id = JStreamIDPing;
    metadata.msg_type_id = JMSGTypeID_PING;
...
- (void)sendPlay{
    ...
    RTMPChunk_0 metadata = {0};
    metadata.msg_stream_id = JStreamIDPlay;
    metadata.msg_type_id = JMSGTypeID_INVOKE;
